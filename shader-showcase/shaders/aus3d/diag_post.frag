#version 460
// TEST: Post-processing style grayscale 3D sphere
...
void main(){
    ...
    // Sample checker texture, convert to grayscale
    vec2 suv=vec2(atan(P.z,P.x)*0.1591549+0.5,acos(clamp(P.y,-1.0,1.0))*0.3183099);
    vec3 tex=texture(uInputTex,suv).rgb;
    float gray=dot(tex,vec3(0.299,0.587,0.114))*P0;
    // Tint: warm lit side, cool shadow
    float ndl=dot(N,L)*0.5+0.5;
    vec3 col=vec3(gray)*mix(vec3(0.6,0.7,0.9),vec3(1.0,0.95,0.8),ndl)*uLightColor;
    outColor=vec4(col,1);
}
