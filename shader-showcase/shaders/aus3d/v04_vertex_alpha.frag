#version 460
// Vol.04-16 VertexLight+Alpha: vertex-lighting with adjustable alpha test
...
void main(){
    ...
    // Stencil-style alpha test using hash
    float alpha=hash(floor(P.yz*5.0+0.5));
    float threshold=P0;
    if(alpha<threshold)outColor=vec4(0);
else outColor=vec4(vec3(P1,P2,P3)*ndl*uLightColor,1);
}