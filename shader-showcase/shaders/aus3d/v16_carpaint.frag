...
// EFFECT_CODE belowvec2 matcapUV=vec2(dot(N,rt)*0.5+0.5,dot(N,up)*0.5+0.5);vec3 matcap=texture(uInputTex,matcapUV).rgb;vec3 col=mix(matcap*1.2,matcap,P0);outColor=vec4(col,1);
}
