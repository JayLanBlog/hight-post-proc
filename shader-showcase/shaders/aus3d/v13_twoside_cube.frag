#version 460
// Vol.13 TwoSide Cube — same FOV as sphere
...
void main(){
    ...
    vec3 L=normalize(uLightDir);vec3 V=normalize(eye-P);
    float alpha=clamp(P0,0.1,1.0);
    bool front=dot(N,V)>0.0;
    vec3 col=front?vec3(0.2,0.4,0.9):vec3(0.9,0.4,0.1);
    float ndl=abs(dot(N,L))*0.5+0.5;col*=0.3+0.7*ndl;
