#version 460
// Vol.04-4 AlphaTest: discard pixels below alpha threshold
...
void main(){
    ...
    // Checkerboard-style alpha test using hash
    float alpha=hash(floor(P.xz*6.0+0.5));
    float threshold=P0;
    if(alpha<threshold)outColor=vec4(0);
else outColor=vec4(vec3(0.2,0.8,0.3)*ndl*uLightColor,1);
}