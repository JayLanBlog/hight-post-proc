#version 460
// Vol.05-23 Glass v3: cubemap reflection + specular + alpha transparency
...
void main(){
    ...
    vec3 refl=mix(vec3(0.8,0.9,1.0),vec3(0.3,0.6,1.0),skyGrad);
    float fresnel=0.04 + 0.96 * pow(1.0-abs(dot(N,V)), 5.0);

    vec3 L=normalize(uLightDir);
    float ndl=dot(N,L)*0.5+0.5;
    vec3 diffuse=vec3(P0,P1,P2)*ndl;
    ...
    specColor+=specGlow * vec3(0.90,0.90,0.95) * 2.0;
    specColor+=specWide * vec3(0.85,0.85,0.90) * 0.5;

    vec3 col=mix(diffuse,refl,fresnel);
    col += specColor;

    float alpha=clamp(P3 * (1.0 - P0) + 0.2, 0.15, 0.95);
