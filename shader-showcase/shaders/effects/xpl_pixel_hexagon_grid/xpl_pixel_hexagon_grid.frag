#version 460
// Hexagon Grid — honeycomb grid, clamped to image edges
...
void main() {
    ...
    // Hex grid border
    float grid=1.0-smoothstep(0.55,0.72,edgeDist);
    grid=clamp(grid,0,1)*clamp(uParamFloat1,0,2);
    outColor=mix(col,vec4(0,0,0,1),grid);
}
