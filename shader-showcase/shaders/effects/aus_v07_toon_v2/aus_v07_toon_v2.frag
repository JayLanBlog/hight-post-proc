#version 460
layout(location=0) in vec3 vWorldPos;
layout(location=1) in vec3 vWorldNormal;
layout(location=2) in vec2 vUV;
layout(location=3) in vec3 vEyeDir;
layout(location=4) in vec3 vLightDir;
layout(location=0) out vec4 outColor;
layout(binding=0) uniform sampler2D uInputTex;
layout(std140, binding=1) uniform Params {
    float uParamFloat0,uParamFloat1,uParamFloat2,uParamFloat3,uParamFloat4,uParamFloat5;
    vec2 uResolution; float uTime; float uFrameCount;
    mat4 uMVP; mat4 uModelView;
    vec3 uLightDir; float _p0; vec3 uLightColor; float _p1; vec3 uEyePos; float _p2;
};

void main() {
    vec3 N = normalize(vWorldNormal);
    vec3 L = normalize(vLightDir);
    vec3 V = normalize(vEyeDir);
    vec3 H = normalize(L + V);
    float NdotL = dot(N, L) * 0.5 + 0.5;
    float NdotH = max(dot(N, H), 0.0);
    float levels = max(uParamFloat0, 2.0);
    float toon = floor(NdotL * levels) / levels;
    float spec = step(uParamFloat1, pow(NdotH, 32.0));
    vec3 col = vec3(uParamFloat2, uParamFloat3, uParamFloat4);
    outColor = vec4(col * uLightColor * (toon * 0.8 + 0.2) + vec3(1)*spec*0.5, 1.0);
}
