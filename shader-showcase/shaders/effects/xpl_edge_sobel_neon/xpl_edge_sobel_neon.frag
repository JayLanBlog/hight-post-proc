...
void main() {
vec2 step=vec2(uParamFloat0,uParamFloat1)/uResolution;float g=Sobel(step,vUV);
vec4 sc=texture(uInputTex,vUV);float pulse=3.0+1.0*sin(uTime*1.0);
outColor=vec4(sc.rgb+vec3(uParamFloat2,uParamFloat3,uParamFloat4)*g*pulse,1.0);
}