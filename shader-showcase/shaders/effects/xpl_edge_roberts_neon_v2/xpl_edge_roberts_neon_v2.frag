...
float tl=I(texture(uInputTex,vUV+vec2(-ts.x,ts.y))),tr=I(texture(uInputTex,vUV+vec2(ts.x,ts.y)));
float bl=I(texture(uInputTex,vUV+vec2(-ts.x,-ts.y))),br=I(texture(uInputTex,vUV+vec2(ts.x,-ts.y)));
float g=sqrt((-tl+br)*(-tl+br)+(-tr+bl)*(-tr+bl));
float pulse=uParamFloat5*(1.0+0.3*sin(uTime*0.9));
outColor=vec4(vec3(uParamFloat2,uParamFloat3,uParamFloat4)*g*pulse,1.0);
}