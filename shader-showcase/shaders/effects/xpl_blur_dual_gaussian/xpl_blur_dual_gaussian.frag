#version 460
// xpl_blur_dual_gaussian — FULL-SCREEN strong blur, sin fade
...
void main() {
    ...
    vec4 blurred = orig;
    blurred = orig * 0.35
        + texture(uInputTex,vUV+vec2(0.0,ps.y)) * 0.13
        + texture(uInputTex,vUV-vec2(0.0,ps.y)) * 0.13
        + texture(uInputTex,vUV+vec2(ps.x,0.0)) * 0.13
        + texture(uInputTex,vUV-vec2(ps.x,0.0)) * 0.13
        + texture(uInputTex,vUV+ps*2.0) * 0.065
        + texture(uInputTex,vUV-ps*2.0) * 0.065;
