...
void main() {
vec2 q=vUV-vec2(uParamFloat1,uParamFloat2)-0.02*vec2(cos(uTime*0.4),sin(uTime*0.3));
float d=dot(q,q)*clamp(uParamFloat0,0,5);float v=clamp(1.0-d,0,1);
outColor=vec4(texture(uInputTex,vUV).rgb*v,1.0);
}