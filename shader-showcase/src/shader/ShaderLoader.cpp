...
std::string ShaderLoader::FindShaderDir() {
    ...
    if (len > 0 && len < MAX_PATH) {
        ...
        // Try: ../../../shaders (for build/bin/Release -> shader-showcase/shaders)
        //      ../../shaders     (for build/bin -> shader-showcase/build/shaders)
        //      ../shaders        (for build -> shader-showcase/shaders)
        for (const char* rel : {"../../../shaders", "../../shaders", "../shaders", "shaders"}) {
            std::string candidate = exeDir + "/" + rel;
